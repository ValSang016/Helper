...
router.get('/', function (req, res, next) {
  res.render('members/member', {title : '회원관리'});
});

router.get('/join', function(req, res, next) { // member/join
  res.render('members/join', { title :'회원가입' });
});
...
router.get('/login', function (req, res, next) {
  res.render('members/login', { title : '로그인'});
});
...
router.post('/login', function (req, res, next){
  ...
  db_member.login(datas, function (callback) {
    if(!callback) {
      ...
      res.render('members/main', {"title" : "개인 메뉴", "id" : req.body.id});
    }
  });
});
...
router.get('/update/:id', function (req, res, next) {
  ...
  db_member.updateform(datas, function (data) {
    ...
    if(data)
      res.render('members/updateform', {"title" : "정보 수정", "row" : data})
  });
});
...
router.post('/update/:id', function (req, res, next) {
  ...
  db_member.update(datas, function (callback) {
    if (callback) {
      res.render('members/main', {"title" : "회원관리", "id" : id});
    }else{
      res.end('<head><meta charset="utf-8"><script> alert("에러가 발생해서 되돌아갑니다!"); ' +
      'history.back(); </script></head>');
    }
...
router.get('/withdraw/:id', function (req, res, next){
  var id = req.params.id;
  res.render('members/withdrawform', {"id" : id, "title" : "회원 탈퇴"});
});
